...
class ReflectorMacro {
	...
	static function checkType():Array<Field> {
		...
		var ex = macro __TYPE_DESC = new inject.def.TypeDescription($v{noConstructor});
		...
		var descField:Field = {
			name: "__TYPE_DESC",
			kind: FieldType.FVar(ComplexType.TPath({pack: ["inject", "def"], name: "TypeDescription"})),
			pos: firstPos,
			access: [Access.AStatic]
		};
