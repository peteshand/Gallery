package inject.reflection;

import haxe.rtti.Meta;
import haxe.xml.Access;
import inject.def.ConstructorInjectionPoint;
import inject.def.MethodInjectionPoint;
import inject.def.NoParamsConstructorInjectionPoint;
import inject.def.PostConstructInjectionPoint;
import inject.def.PreDestroyInjectionPoint;
import inject.def.PropertyInjectionPoint;
import inject.def.TypeDescription;

@:keepSub
class RuntimeTypeReflector implements Reflector {
	var _currentFactoryXML:Xml;
	var _currentFactoryXMLFast:Access;
	var constructorElem:Access;
	var rtti:String;
	var extendPath:String;

	static var whitelist = new Map<String, Bool>();

	var extendDescribeTypeReflector:RuntimeTypeReflector;
	var extendTypeDescription:TypeDescription;
	...
	public function describeInjections(_type:Class<Dynamic>):TypeDescription {
		...
		if (extendDescribeTypeReflector == null) {
			extendDescribeTypeReflector = new RuntimeTypeReflector();
		}
	...
	function inWhitelist(type:Class<Dynamic>):Bool {
	...
	//----------------------         / Protected Methods        ----------------------//
	function addCtorInjectionPoint(description:TypeDescription, type:Class<Dynamic>):Void {
	...
	/*function extractNodeParameters(args:XMLList):Map<Dynamic,Dynamic>
		{
	...
	function addFieldInjectionPoints(description:TypeDescription, type:Class<Dynamic>):Void {
	...
	function addMethodInjectionPoints(description:TypeDescription, type:Class<Dynamic>):Void {
		// FIX
	...
	function addPostConstructMethodPoints(description:TypeDescription, type:Class<Dynamic>):Void {
	...
	function addPreDestroyMethodPoints(description:TypeDescription, type:Class<Dynamic>):Void {
	...
	function gatherOrderedInjectionPointsForTag(injectionPointType:Class<Dynamic>, tag:String, type:Class<Dynamic>):Array<Dynamic> {
