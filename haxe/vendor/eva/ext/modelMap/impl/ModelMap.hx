...
class ModelMap implements IModelMap implements DescribedType {
	...
	public function new(context:IContext) {
		...
		if (initialized)
			return;
		initialized = true;
		var descriptor:Dynamic = untyped Object.getOwnPropertyDescriptor(Browser.window, "notifiers");
		if (descriptor == null || descriptor.configurable == true) {
			untyped Object.defineProperty(Browser.window, "notifiers", {
				get: listModels,
				configurable: true
			});
		}
	...
	public function map(type:Class<Dynamic>, key:String = null):InjectionMapping {
		...
		if (timerActive == false) {
			timerActive = true;
			bindingTimer = Timer.delay(addToWindow, 100);
		}
		types.push({
			type: type,
			key: key
		...
		for (modelDef in types) {
			...
			for (field in Reflect.fields(instance)) {
				...
				if (Std.isOfType(prop, Notifier)) {
					// Haxe properties compile to methods; expose a real JS accessor for DevTools.
					var notifier:Notifier<Dynamic> = cast prop;
					untyped Object.defineProperty(notifier, "value", {
						get: function() return notifier.value,
						set: function(value:Dynamic) { notifier.value = value; },
						configurable: true
					});
	...
	#end
	@preDestroy public function dispose():Void {
		disposed = true;
		if (bindingTimer != null) bindingTimer.stop();
		types = [];
	}
}
