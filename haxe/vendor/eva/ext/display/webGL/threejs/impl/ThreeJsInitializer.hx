package eva.ext.display.webGL.threejs.impl;

import eva.ext.contextView.ContextView;
import eva.ext.display.base.impl.BaseInitializer;
...
class ThreeJsInitializer extends BaseInitializer {
	...
	override public function addLayer(ViewClass:Class<Dynamic>, index:Int, total:Int, id:String):Void {
		...
		if (index == -1)
			layers.addLayer(threeJsLayer);
		else
			layers.addLayerAt(threeJsLayer, index);
	}
}
