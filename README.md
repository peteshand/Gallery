...
Versioned Windows and Android packages are generated in the local `dist/` directory, which is excluded from Git. The latest Windows standalone is `Gallery-0.11.0-Windows.exe`. The Windows installer and Android APK remain at 0.10.0; the new S3 layout and Takeout metadata support are currently available in the Windows standalone. See [Android instructions](docs/ANDROID.md) and [release history](docs/VERSIONS.md).
...
- **Settings → Cloud storage** stores credentials in Windows Credential Manager. The macOS Keychain adapter passed persistence testing in 0.8.4; the current 0.11.0 source still needs a fresh Mac build and platform acceptance. Android Keystore support compiles and still needs device persistence testing. **Test connection** performs a read-only S3 check.
